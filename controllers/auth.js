...
exports.signin = (req, res) => {
    ...
    User.findOne({email}, (err, user) => {
        //if error or no user
        if(err || !user){
            return res.status(401).json({error:"User with that email not EXISTS, please singup."})
        }

        //if user is found make sure the email and password match
        //create authentication method in DB and use in here
